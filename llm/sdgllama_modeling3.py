...
class SDGLlamaModel(LlamaModel):
    ...
    def __init__(self, config: SDGConfig):
        ...
        self.graph_token_embedding = nn.Parameter(torch.randn(1, 1, config.hidden_size))
        self.text_token_embedding = nn.Parameter(torch.randn(1, 1, config.hidden_size))
    ...
    def structure_attention(self, t, sims, sg_nodes, start=None, end=None, temp=0.2):
        '''
        对 t[:, start:end, :] 的序列区间进行加权平均操作，返回与 t 形状相同的结果。
        ...
        返回:
        t_hat: 加权平均后的 hidden_states, [batch_size, seq_len, embed_dim]
        '''
        if start is None: start = 0
        if end is None: end = t.size(1)
        t_segment = t[:, start:end, :]  # [batch_size, end-start, embed_dim]

        f = lambda x: torch.exp(x / temp)
        filtered_sims = f(sims) * sg_nodes  # [batch_size, batch_size]

        sims_expanded = filtered_sims.unsqueeze(-1).unsqueeze(-1)  # [batch_size, batch_size, 1, 1]
        weighted_t_segment = sims_expanded * t_segment.unsqueeze(1)  # [batch_size, batch_size, end-start, embed_dim]

        sim_sum = filtered_sims.sum(dim=1, keepdim=True).unsqueeze(-1)  # [batch_size, 1, 1]
        sim_sum = sim_sum + (sim_sum == 0).float()

        averaged_segment = (weighted_t_segment.sum(dim=0) / sim_sum).squeeze(-1)  # [batch_size, end-start, embed_dim]

        t_hat = t.clone()  # [batch_size, seq_len, embed_dim]
        t_hat[:, start:end, :] = averaged_segment  # 替换区间 [start:end] 的部分
    ...
    def forward(
        ...
        start = 0
        if struct_encode is not None and struct_encode.size(-1) // self.config.hidden_size > 0:
            batch_size = inputs_embeds.size(0)
            graph_token = self.start_token_embedding.expand(batch_size, -1, -1)  # Expand to batch size
            text_token = self.text_token_embedding.expand(batch_size, -1, -1)
            gpse_token_count = struct_encode.size(-1) // self.config.hidden_size 
            start = gpse_token_count + 2
            gpse_tokens = struct_encode.view(batch_size, gpse_token_count, self.config.hidden_size)

            inputs_embeds = torch.cat([graph_token, gpse_tokens, text_token, inputs_embeds], dim=1)

            if attention_mask is not None:
                additional_mask = torch.ones((batch_size, 2 + gpse_token_count), device=attention_mask.device)
                attention_mask = torch.cat([additional_mask, attention_mask], dim=1)
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if i == 15 and use_sba:
                hidden_states = self.structure_attention(hidden_states, sims, subgraph_nodes, start)
